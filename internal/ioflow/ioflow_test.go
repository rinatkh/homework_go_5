...
func TestReadAllText(t *testing.T) {
	for _, tt := range []struct{ input, want string }{{"", ""}, {"a", "a"}, {"hello", "hello"}, {"line\n", "line\n"}, {"рус", "рус"}, {"123", "123"}, {" space ", " space "}, {strings.Repeat("x", 10), strings.Repeat("x", 10)}, {"a\nb", "a\nb"}, {"!", "!"}} {
...
func TestCountBytes(t *testing.T) {
	for _, tt := range []struct {
		input string
		want  int
	}{{"", 0}, {"a", 1}, {"hello", 5}, {"line\n", 5}, {"рус", 6}, {"123", 3}, {" space ", 7}, {strings.Repeat("x", 10), 10}, {"a\nb", 3}, {"!", 1}} {
...
func TestCopyUpper(t *testing.T) {
	for _, tt := range []struct{ in, want string }{{"", ""}, {"a", "A"}, {"hello", "HELLO"}, {"Go Lang", "GO LANG"}, {"рус", "РУС"}, {"123", "123"}, {" space ", " SPACE "}, {"a-b", "A-B"}, {"a\nb", "A\nB"}, {"!", "!"}} {
		var b bytes.Buffer
...
func TestScanLines(t *testing.T) {
	tests := []struct {
		in   string
		want []string
	}{{"", []string{}}, {"a", []string{"a"}}, {"a\n", []string{"a"}}, {"a\nb", []string{"a", "b"}}, {"\n", []string{""}}, {"a\n\n", []string{"a", ""}}, {"рус\nтекст", []string{"рус", "текст"}}, {" space ", []string{" space "}}, {"1\n2\n3", []string{"1", "2", "3"}}, {"!", []string{"!"}}}
	for _, tt := range tests {
...
func TestFirstLine(t *testing.T) {
	for _, tt := range []struct{ in, want string }{{"", ""}, {"a", "a"}, {"a\n", "a"}, {"a\nb", "a"}, {"\n", ""}, {"\nb", ""}, {"рус\nтекст", "рус"}, {" space ", " space "}, {"1\n2\n3", "1"}, {"!", "!"}} {
...
func TestReadCSVLike(t *testing.T) {
	tests := []struct {
		in   string
		want [][]string
	}{{"", [][]string{}}, {"a", [][]string{{"a"}}}, {"a,b", [][]string{{"a", "b"}}}, {"a, b", [][]string{{"a", "b"}}}, {"a,b\nc,d", [][]string{{"a", "b"}, {"c", "d"}}}, {",", [][]string{{"", ""}}}, {" a , b ", [][]string{{"a", "b"}}}, {"рус,текст", [][]string{{"рус", "текст"}}}, {"1,2,3", [][]string{{"1", "2", "3"}}}, {"!", [][]string{{"!"}}}}
	for _, tt := range tests {
...
func TestLimitRead(t *testing.T) {
	for _, tt := range []struct {
		...
		limit int
		want  string
	}{{"abc", 0, ""}, {"abc", 1, "a"}, {"abc", 2, "ab"}, {"abc", 3, "abc"}, {"abc", 10, "abc"}, {"", 10, ""}, {"abc", -1, ""}, {"рус", 2, "ру"[:2]}, {"line\n", 4, "line"}, {"!", 1, "!"}} {
...
func TestRepeatToWriter(t *testing.T) {
	for _, tt := range []struct {
		...
		count int
		want  string
	}{{"a", 0, ""}, {"a", 1, "a"}, {"a", 2, "aa"}, {"ab", 3, "ababab"}, {"", 5, ""}, {"рус", 2, "русрус"}, {"x", -1, ""}, {"!", 4, "!!!!"}, {" ", 3, "   "}, {"go", 1, "go"}} {
		var b bytes.Buffer
...
func TestReadAndTrim(t *testing.T) {
	for _, tt := range []struct{ in, want string }{{"", ""}, {" a ", "a"}, {"\nhello\n", "hello"}, {"рус ", "рус"}, {" 123 ", "123"}, {" space inside ", "space inside"}, {"\tgo\t", "go"}, {"!", "!"}, {" a\nb ", "a\nb"}, {"   ", ""}} {
